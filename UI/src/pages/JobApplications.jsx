...
import { ChevronDown, Briefcase, MapPin, Wallet, Code, ChevronLeft, ChevronRight, Search, ArrowUpDown, ArrowUp, ArrowDown, X, Building, Tag, Loader2 } from 'lucide-react';
...
const JobPostRow = ({ job }) => {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.getApplicationsByJobId(job.id), {
      ...
      if (result.isSuccess) {
        ...
        toast({ title: 'Error', description: 'Failed to fetch applications for this job.', variant: 'destructive' });
      }
    } catch (error) {
      ...
      >
        <div className="flex-grow">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500 mt-2">
            {job.location && <span className="flex items-center"><MapPin size={12} className="mr-1"/>{job.location}</span>}
            {job.salary && <span className="flex items-center"><Wallet size={12} className="mr-1"/>{job.salary}</span>}
            {job.skils && <span className="flex items-center"><Code size={12} className="mr-1"/>{job.skils}</span>}
          </div>
        </div>
...
const JobApplications = () => {
    ...
    const [filters, setFilters] = useState({ title: '', companyName: '', location: '', skils: '' });
    ...
    const fetchAllJobPosts = useCallback(async (page) => {
        showLoader();
        try {
            const token = sessionStorage.getItem('token');
            ...
            if (result.isSuccess) {
                ...
                toast({ title: 'Error', description: 'Failed to fetch job applications.', variant: 'destructive' });
            }
        } catch (error) {
            toast({ title: 'Network Error', description: 'Could not connect to the server.', variant: 'destructive' });
        } finally {
    ...
    }, [toast, showLoader, hideLoader, navigate, debouncedSearch, debouncedFilters, sortBy, isDescending]);
    ...
    return (
        <>
            <Helmet>
                <title>All Job Applications - QXI HR Pvt Ltd</title>
                <meta name="description" content="Manage and review all job applications." />
            </Helmet>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center mb-12">
                        ...
                        <p className="mt-3 max-w-md mx-auto text-base text-gray-500 sm:text-lg md:mt-5 md:text-xl md:max-w-3xl">
                            Filter, review, and manage applications for all open positions.
                        </p>
                    </div>
                    ...
                    <div className="space-y-4">
                        {jobPosts.length > 0 ? (
                            jobPosts.map(job => <JobPostRow key={job.id} job={job} />)
                        ) : (
