...
import { Plus, Edit, Trash2, X, Filter, Loader2, ChevronLeft, ChevronRight, Save, Image, Tag, ChevronDown, ChevronUp } from 'lucide-react';
...
const CategoryManagement = ({ onCategoryUpdate }) => {
    ...
    const handleUpdate = async (id) => {
        ...
        showLoader();
        try {
            const response = await fetch(API_ENDPOINTS.updateImageCategory(id), {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${sessionStorage.getItem('token')}` },
    ...
    const handleDelete = async (id) => {
        showLoader();
        try {
            const response = await fetch(API_ENDPOINTS.deleteImageCategory(id), {
                method: 'DELETE',
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md mb-8">
            <button onClick={() => setIsExpanded(!isExpanded)} className="w-full flex justify-between items-center text-left">
                <h2 className="text-2xl font-bold text-gray-800 flex items-center"><Tag className="mr-2"/>Category Management</h2>
            ...
            {isExpanded && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} transition={{ duration: 0.3 }} className="overflow-hidden mt-4">
                    <div className="flex gap-4 mb-6">
                        ...
                        <AlertDialog>
                            <AlertDialogTrigger asChild>
                                <Button className="corporate-gradient text-white" disabled={!newCategoryName.trim()}><Plus className="mr-2 h-4 w-4"/>Add Category</Button>
                            </AlertDialogTrigger>
                            <AlertDialogContent>
                                <AlertDialogHeader><AlertDialogTitle>Create Category?</AlertDialogTitle><AlertDialogDescription>Are you sure you want to create this category?</AlertDialogDescription></AlertDialogHeader>
                                <AlertDialogFooter>
                    ...
                    <div className="space-y-2">
                        {categories.map(cat => (
                            <div key={cat.id} className="flex items-center justify-between p-2 rounded-md bg-gray-50">
                                ...
                                <div className="flex gap-2">
                                    {editingCategoryId === cat.id ? (
                                        <>
                                            <AlertDialog>
                                                <AlertDialogTrigger asChild>
                                                    <Button size="icon" variant="ghost" disabled={!editingCategoryName.trim()}><Save className="h-4 w-4 text-green-600"/></Button>
                                                </AlertDialogTrigger>
                                                <AlertDialogContent>
                                                    <AlertDialogHeader><AlertDialogTitle>Update Category?</AlertDialogTitle><AlertDialogDescription>Are you sure you want to update this category?</AlertDialogDescription></AlertDialogHeader>
                                                    <AlertDialogFooter>
                                                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                                                        <AlertDialogAction onClick={() => handleUpdate(cat.id)}>Update</AlertDialogAction>
                                                    </AlertDialogFooter>
                                                </AlertDialogContent>
                                            </AlertDialog>
                                        ...
                                        <Button size="icon" variant="ghost" onClick={() => { setEditingCategoryId(cat.id); setEditingCategoryName(cat.name); }}><Edit className="h-4 w-4"/></Button>
                                    )}
                                     <AlertDialog>
                                        ...
                                        <AlertDialogContent>
                                            <AlertDialogHeader><AlertDialogTitle>Delete Category?</AlertDialogTitle><AlertDialogDescription>This will delete "{cat.name}". Are you sure?</AlertDialogDescription></AlertDialogHeader>
                                            <AlertDialogFooter>
                                                <AlertDialogCancel>Cancel</AlertDialogCancel>
...
const Gallery = () => {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      toast({ title: 'Network Error', description: 'Could not connect to fetch categories.', variant: 'destructive' });
    }
  }, [toast]);

  const fetchGalleryImages = useCallback(async (page, categoryId) => {
    setIsLoading(true);
    showLoader();
    try {
      let response;
      if (categoryId === 'all') {
        ...
        response = await fetch(API_ENDPOINTS.getGalleryImagesByCategory(categoryId), {
           headers: { 'Authorization': `Bearer ${sessionStorage.getItem('token')}` }
        });
      }
      ...
      toast({ title: 'Network Error', description: 'Could not connect to fetch images.', variant: 'destructive' });
    } finally {
      setIsLoading(false);
      hideLoader();
    }
  ...
  const handleFormSubmit = async (itemData) => {
    ...
    try {
        ...
        if (itemData.imageFile) {
            const getUrlResponse = await fetch(API_ENDPOINTS.getUploadUrl(itemData.imageFile.name), {
                headers: { 'Authorization': `Bearer ${sessionStorage.getItem('token')}` }
            });
        ...
        const endpoint = isEditing ? API_ENDPOINTS.updateGalleryImage(editingItem.id) : API_ENDPOINTS.createGalleryImage;
        ...
        const result = await response.json();
        if (result.isSuccess) {
            toast({ title: 'Success', description: `Gallery item ${isEditing ? 'updated' : 'created'}.` });
            setIsModalOpen(false);
            fetchGalleryImages(currentPage, activeFilter);
        } else {
            toast({ title: 'Error', description: result.message || 'Failed to save item.', variant: 'destructive' });
        }
    } catch (error) {
  ...
  return (
    <>
      <Helmet>
        <title>Gallery - QXI HR Pvt Ltd</title>
        <meta name="description" content="Explore our gallery of achievements, team moments, and professional milestones." />
      </Helmet>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              <h1 className="text-4xl md:text-5xl font-bold mb-6 text-shadow">Our Gallery</h1>
              <p className="text-xl md:text-2xl max-w-3xl opacity-90">Showcasing achievements, team, and memorable moments</p>
            </motion.div>
            {(userRole === 'Admin' || userRole === 'Staff') && (
              <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
                <Button onClick={openAddModal} size="lg" className="bg-white text-gray-900 hover:bg-gray-100">
                  <Image className="w-5 h-5 mr-2" />Add Image
                </Button>
              </motion.div>
            )}
          </div>
      ...
      <section className="py-8 bg-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {(userRole === 'Admin' || userRole === 'Staff') && <CategoryManagement onCategoryUpdate={fetchCategories} />}
          <div className="flex flex-wrap gap-2 justify-center">
            {categories.map((category) => (
              <Button key={category.id} variant={activeFilter === category.id ? "default" : "outline"} onClick={() => handleFilterChange(category.id)} className={`${activeFilter === category.id ? 'corporate-gradient text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {isLoading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-600 mb-8">There are no images in this category yet.</p>
              {(userRole === 'Admin' || userRole === 'Staff') && <Button onClick={openAddModal} size="lg" className="corporate-gradient text-white"><Image className="w-5 h-5 mr-2" />Add Image</Button>}
            </div>
          ) : (
              ...
              {galleryItems.map((item, index) => (
                <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.1 }} className="bg-white rounded-xl shadow-lg overflow-hidden group">
                  <div className="relative overflow-hidden">
                    ...
                    {(userRole === 'Admin' || userRole === 'Staff') && (
                      <div className="absolute top-4 right-4 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        <Button size="icon" variant="secondary" onClick={() => openEditModal(item)} className="w-8 h-8 bg-white/90 hover:bg-white"><Edit className="w-4 h-4" /></Button>
                         <AlertDialog>
                            <AlertDialogTrigger asChild><Button size="icon" variant="destructive" className="w-8 h-8"><Trash2 className="w-4 h-4" /></Button></AlertDialogTrigger>
                            <AlertDialogContent>
                                <AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>This will permanently delete the gallery image.</AlertDialogDescription></AlertDialogHeader>
                                <AlertDialogFooter>
                                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-gray-600 text-sm leading-relaxed" style={{ whiteSpace: 'pre-line' }}>{item.description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <GalleryModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleFormSubmit} item={editingItem} categories={categories.filter(c => c.id !== 'all')} />
    </>
  );
};
